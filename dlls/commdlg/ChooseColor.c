...

{
  ...
  if ( ! ( WCCStruct.hColorCursor = LoadCursor ( TWIN_GetInstance (), MAKEINTRESOURCE ( IDC_COLORCROSSHAIR ) ) ) )
    ErrorCode = CDERR_LOADRESFAILURE;
  else
  if ( ! ( WCCStruct.hIconColorCursor = LoadIcon ( TWIN_GetInstance (), MAKEINTRESOURCE ( IDI_COLORCROSSHAIR ) ) ) )
    ErrorCode = CDERR_LOADRESFAILURE;
  else
  if ( ! ( WCCStruct.hLuminosityCursor = LoadCursor ( TWIN_GetInstance (), MAKEINTRESOURCE ( IDC_LUMINOSITY ) ) ) )
    ErrorCode = CDERR_LOADRESFAILURE;
  else
  if ( ! ( WCCStruct.hIconLuminosityCursor = LoadIcon ( TWIN_GetInstance (), MAKEINTRESOURCE ( IDI_LUMINOSITY ) ) ) )
    ErrorCode = CDERR_LOADRESFAILURE;
  else
  {
...

{
    ...
    if (( WCCStruct.lpChooseColor->Flags & CC_ENABLEHOOK ) && 
        ...
        rc = CallWindowProc( (FARPROC)WCCStruct.lpChooseColor->lpfnHook,
...

{
  ...
  if ((Message != WM_INITDIALOG) &&
	(ErrorCode = WCCCallHook( hWnd, Message, wParam, lParam )) )
  ...
  switch ( Message )
  {
    ...
    case WM_COMMAND:
      ...
      switch ( id )
      {
        ...
        case WCCEdtColorGreen:
        case WCCEdtColorBlue:
          WCFGetEditControlValueAndSetState ( hWnd, id );
          break;
...
#ifdef TWIN32
		switch ( ReturnedCode = DialogBoxIndirectParam ( TWIN_GetInstance (), lpResource,
                                                        WCCStruct.lpChooseColor->hwndOwner,
                                                        ( DLGPROC )WCCDialogProc, 0L ) )
#else
		switch ( ReturnedCode = DialogBoxIndirectParam ( TWIN_GetInstance (), hResDialog,
                                                        WCCStruct.lpChooseColor->hwndOwner,
                                                        ( DLGPROC )WCCDialogProc, 0L ) )
#endif
      {
