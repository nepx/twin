...
HINSTANCE TWIN_GetInstance(void);
...

{
  ...
  if ( ! LoadString ( TWIN_GetInstance (), StringID, ( LPSTR ) String,
                      sizeof ( String ) ) )
...

{
  ...
  if ( ! ( LoadString ( TWIN_GetInstance (), WCF_IDS_SAMPLETEXT,
                        ( LPSTR ) SampleText, sizeof ( SampleText ) ) ) )
    ErrorCode = CDERR_LOADSTRFAILURE;
  else
  if ( ! ( hTrueTypeBitmap = LoadBitmap ( TWIN_GetInstance (),
                                          MAKEINTRESOURCE ( WCF_IDB_TRUETYPE ) ) ) )
    ErrorCode = CDERR_FINDRESFAILURE;
  if ( ! ( hPrinterBitmap = LoadBitmap ( TWIN_GetInstance (),
...

{
  ...
  if ( ( lpCurrentChooseFont ) &&
    ...
    rc =  ( LOWORD (CallWindowProc((FARPROC)lpCurrentChooseFont->lpfnHook,
...

{
  ...
  if ( SendMessage ( hFontNameWnd, CB_SETITEMDATA, ItemIndex,
                      lpEnumFontData->PrinterFonts ? (LPARAM)hPrinterBitmap :
                      lpNTM->tmPitchAndFamily & TMPF_TRUETYPE ?
                        (LPARAM)hTrueTypeBitmap : FALSE ) == CB_ERR )
    ErrorCode = CDERR_INITIALIZATION;
...

{
  ...
  for ( ColorIndex = 0; ( ! ErrorCode ) && ( ColorIndex < NUM_COLORS );
        ColorIndex++ )
  {
    if ( ! ( LoadString ( TWIN_GetInstance (), ColorData [ ColorIndex ].StringID,
                          ( LPSTR ) ColorString,
          sizeof ( ColorString ) ) ) )
      ErrorCode = CDERR_LOADSTRFAILURE;
    else
...

{
  ...
  if ((! ErrorCode ) && ( Size ) &&
        ( ! ( ErrorCode = WCFGetSelectedItemData ( hStyleWnd,
       	...
       	( SelectedFontStyle ) &&
        ( !(ErrorCode = 
	   WCFGetSelectedItemData ( GetDlgItem ( hDialogWnd, WCFCBFontColor ),
                                                    ( LRESULT FAR* )&Color)))) {
...

{
  ...
  if ( (Message != WM_INITDIALOG ) && 
	( ErrorCode = WCFCallHook ( hWnd, Message, wParam, lParam ) ))
	return ( ErrorCode );
	...
	ReturnValue = 1;
      ...
      case WM_INITDIALOG:
        ...
        if ( lpCurrentChooseFont->Flags & CF_INITTOLOGFONTSTRUCT )
        {
          ...
          WCFDisplayFontData ( hWnd );
          if ( LoadString ( TWIN_GetInstance (), ResourceID, String,
                            sizeof ( String ) ) )
        ...
        WCFCallHook( hWnd, Message, wParam, lParam );
	ReturnValue = 1;
        break;
      ...
      case WM_COMMAND:
        ...
        switch ( id )
        {
          ...
          case IDCANCEL:
          case IDOK:
            PostMessage ( hWnd, WM_CLOSE, id, ( LPARAM )NULL );
            ReturnValue = 1;
            break;
        }
        break;
  ...
  else
  {
    ...
    { /*-- Get Dialog from resource --*/
      ...
      { /*-- Use default dialog in library resource section
              NOTE that hUseInstance is set --*/
        if ( ! ( hResource = FindResource ( hUseInstance = TWIN_GetInstance (),
                                            CHOOSEFONTDIALOG,
                                            RT_DIALOG ) ) )
          ErrorCode = CDERR_FINDRESFAILURE;
      }
...
#ifdef TWIN32
		switch ( ReturnedCode = DialogBoxIndirectParam( TWIN_GetInstance (),
                                   lpResource,
                                   lpCurrentChooseFont->hwndOwner,
