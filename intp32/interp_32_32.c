...
BYTE *
hsw_interp_32_32(ENVPARAMS, BYTE *P0, register BYTE *__PC, int *err, int cycmax)
{
    BYTE *mem_ref;
...
/*a5*/	case MOVSw: {
	    ...
	    temp = FETCH_QUAD(src);
	    PUT_QUAD(dest, temp);
	    (iENV->flags & DIRECTION_FLAG)?(ESI-=4,EDI-=4):(ESI+=4,EDI+=4);
	    } PC += 1; goto next_switch;
/*a6*/	case CMPSb: {
	    DWORD src1, src2; int res;
	    BYTE *src, *dest;
...
/*ab*/	case STOSw:
	    ...
	    (iENV->flags & DIRECTION_FLAG)?(EDI-=4):(EDI+=4);
	    PC += 1; goto next_switch;
/*ac*/	case LODSb: {
	    BYTE *seg;
	    seg = OVERR_DS + (ESI);
...
/*ad*/	case LODSw: {
	    ...
	    seg = OVERR_DS + (ESI);
	    EAX = FETCH_QUAD(seg);
	    (iENV->flags & DIRECTION_FLAG)?(ESI-=4):(ESI+=4);
	    } PC += 1; goto next_switch;
/*ae*/	case SCASb: {
	    DWORD src1, src2; int res;
	    src1 = AL;
...
/*----------IA--------------------------------*/
		...
		case ADDRoverride:	/* 0x67 */
		    PC=P0;	/* re-parse all */
		    code32 = 0;
		    PC = hsw_interp_32_16 (ENVARGS, P0, PC, err);
		    code32 = 1;
		    if (*err) return (*err==EXCP_GOBACK? PC:P0);
		    goto next_switch;
/*----------IA--------------------------------*/
