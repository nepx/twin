...
#ifndef DOSEMU
/* This is for Twin - for dosemu version look into emu-ldt.c */
...
int SetSegreg(unsigned char **lp, unsigned char *big, unsigned long csel)
{
	...
	if (sel < 4 /*(sel >> 3) == 0*/) {
	    if ((csel==MK_CS)||(csel==MK_SS)) {
		e_printf("LDT: invalid selector %#x\n", sel);
	        e = EXCP0D_GPF;
	    }
	    /* else allow zero selectors for ES,FS,GS */
	}
	else if ((sel & 4) == 0) {
	    /* well, no need of checking for GDT segments in Twin... */
	    e_printf("LDT: invalid selector %#x\n", sel);
	    e = EXCP0D_GPF;
	}
	else {
	    ...
	    if (!(wFlags & DF_PRESENT)) {
	        if (!LoadSegment(sel)) {	/* this is NOT in dosemu! */
		    e_printf("LDT: failed to load selector %x\n",sel);
		    e = (csel==MK_SS? EXCP0C_STACK : EXCP0B_NOSEG);
		    goto quit;
		}
		/* else ok, go ahead with the new loaded segment */
	    }
...
quit:
	if (e) {
	    char prnt_buf[40];
	    sprintf(prnt_buf,
	       "INTP32: fault %d for selector %x\n",e,(unsigned int)sel);
	    printf("%s\n",prnt_buf);
	    FatalAppExit(0,prnt_buf);	/* dosemu returns the fault code */
	}
	return 0;
}
#endif
...
static char *e_emu_disasm(ENVPARAMS, unsigned char *org, int is32)
{
...
static char *e_print_cpuemu_regs(ENVPARAMS, int is32)
{
	static char buf[512];
#ifdef TRACE_STACK
	void *sp;
	...
	*buf = 0;
	if (d_emu>3) {
	  long lflags;
	  strcpy(buf,eregbuf);
	  exprintl(iENV->rax.e,buf,0x05);
	  exprintl(iENV->rbx.e,buf,0x12);
	  exprintl(iENV->rcx.e,buf,0x1f);
	  exprintl(iENV->rdx.e,buf,0x2c);
	  exprintl(iENV->rsi.esi,buf,0x3a);
	  exprintl(iENV->rdi.edi,buf,0x47);
	  exprintl(iENV->rbp.ebp,buf,0x54);
	  exprintl(iENV->rsp.esp,buf,0x61);
	  exprintl(iENV->trans_addr,buf,0x6f);
	  exprintw(iENV->cs.cs,buf,0x7c);
	  exprintw(iENV->ds.ds,buf,0x89);
	  exprintw(iENV->es.es,buf,0x96);
	  exprintw(iENV->fs.fs,buf,0xa4);
	  exprintw(iENV->gs.gs,buf,0xb1);
	  exprintw(iENV->ss.ss,buf,0xbe);
	  lflags = (iENV->flags&0x3ff73e)|(CARRY&1)|(RES_8&0x80)|(IS_ZF_SET<<6)|
	  	(IS_OF_SET<<11);
	  ...
	  if (VM86F) {
	    ...
	    sp = (void *)(LONG_SS + (BIG_SS? iENV->rsp.esp:iENV->rsp.sp.sp));
	    p += sprintf(p,"\tstk=");
	    if ((unsigned long)sp > (unsigned long)GetSelectorAddrMax(iENV->ss.ss)) {
	    ...
	    if (code32) {
	      ...
	      for (i=0; i<6; i++) {
	        p += sprintf(p," %08lx",FETCH_QUAD(sp));
	        sp += 4;
	      }
	      ...
	      for (i=0; i<10; i++) {
#ifdef ULTRA_DEBUG
	        p += sprintf(p," %02x%02x",((char *)sp)[1],((char *)sp)[0]);
#else
	        p += sprintf(p," %04x",FETCH_WORD(sp));
#endif
	        sp += 2;
	      }
	      for (; i<10; i++) p+=sprintf(p," ****");
...
static char *e_print_internal_regs(ENVPARAMS)
{
#ifdef ONESTRUCT
	Interp_ENV *v=env;
#else
	Interp_VAR *v=interp_var;
#endif
...
void e_trace (ENVPARAMS, unsigned char *dP0, unsigned char *dPC,
  	int is32)
{
	if ((d_emu>2) && TRACE_HIGH && (dP0==dPC)) {
	    e_printf("%ld,%ld,%d\n%s%s    %s\n",iENV->level,instr_count,is32,
		((((*dPC&0xf8)==0xd8) || (CEmuStat & CeS_LOCK)) ?
		    "" :
		    e_print_cpuemu_regs(ENVARGS,is32)),
		e_print_internal_regs(ENVARGS),
		e_emu_disasm(ENVARGS,dPC,is32));
	    isEMUon = 1;
	}
}
