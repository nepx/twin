...
extern LRESULT  CALLBACK DefLISTBOXProc(HWND, UINT, WPARAM, LPARAM);
...
static LRESULT CALLBACK DefCBProc(HWND , UINT , WPARAM , LPARAM );
...
DefCOMBOBOXProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KILLFOCUS:
        ...
        */
        if ((lp->wStyle & 0x0F) != CBS_SIMPLE && wParam != (UINT)lp->ListBoxControl && wParam != 0)
...
/* ecw */  SendMessage(lp->hWndParent, WM_COMMAND, GET_WM_COMMAND_MPS(lp->nID, hWnd, CBN_SELENDOK));
    ...
    case WM_NCDESTROY:
        if ((lp->wStyle & 0xf) != CBS_SIMPLE && IsWindow(lp->ListBoxControl))
           DestroyWindow(lp->ListBoxControl);
        SetWindowLong(hWnd, CWD_LPCBDATA, 0L);
        WinFree((LPSTR)lp);
        return 0L;
    ...
    case WM_LBUTTONDOWN:
        ...
        if (!IS_SET(lp, CSF_CAPTUREACTIVE)) /* no listbox yet */
		  ...
		  if (lp->ListBoxRect.bottom - lp->ListBoxRect.top <
		    ...
		    /*
		     * Modified by Gilles Depeyrot 1999/07/27
		     *   the frame size is not taken into account ? added 10
		     */
		    lp->ListBoxRect.bottom =
		      lp->ListBoxRect.top + ((lp->uHeight-2) * nitems) + 10;
...
DefCBProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(uMsg) {
        ...
        case CB_SETEDITSEL:
            rc = SendMessage(lp->EditControl,EM_SETSEL,
                wParam,lParam);
#ifdef TWIN32
            rc = SendMessage(lp->EditControl,EM_SCROLLCARET,
                wParam,lParam);
#endif
	    return rc;
