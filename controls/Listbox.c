...
DefLISTBOXProc(HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_DESTROY:
		...
		hMenuBox = lpLBoxInfo->ObjHead.hObj;

		/* release our reference to it...*/
    		RELEASELBOXINFO(lpLBoxInfo);
	...
	case WM_SETFOCUS:
	    ...
	    /* Change selection to first item if none, and notify parent.
	     * Don't change sel if CTRL or SHIFT is down in EXTENDEDSEL style.
	     */
	    if (wIndex == (WORD)-1) {
		if ( !(lpLBoxInfo->wLBoxStyle & LBS_EXTENDEDSEL) ||
	...
	case WM_CHAR:
	    hWndTmp = GetWindow(hWnd,GW_OWNER);
	    if (hWndTmp && (hWndTmp32 = GETHWND32(hWndTmp))) {
		/* make sure this is a frame handle */
		if (hWndTmp32->lpfnWndProc == FrameProc) {
		    /* if menu exist, pass this message to MenuBarProc */
		    if (!(hWndTmp32->dwStyle & WS_CHILD) &&
			GetWindowMenu(Get32FrameClient(hWndTmp32))) {
			RELEASEWININFO(hWndTmp32);
			return MenuBarProc(hWndTmp,wMsg,wParam,lParam);
		    }
		}
		RELEASEWININFO(hWndTmp32);
	    }
	...
	case WM_KEYDOWN:
		...
		switch (wParam) {
			...
			break;
		    case VK_END:
			wIndex = lpLBoxInfo->wItemCount-1;
			break;
		    case VK_PRIOR:
			wIndex = lpLBoxInfo->wSelection;
			if ((wIndex != (WORD) -1))
			   wIndex = max(0, wIndex - lpLBoxInfo->wFrameSize+1);
			break;
		    case VK_NEXT:
			wIndex = lpLBoxInfo->wSelection;
			if ((wIndex != (WORD) -1))
			   wIndex = min(lpLBoxInfo->wItemCount-1, 
				 	wIndex + lpLBoxInfo->wFrameSize-1);
			break;
		    default:
			return 1L;
		}
	    ...
	    {
		WORD wIndexOld = lpLBoxInfo->wSelection;
	        LBoxChangeSelection(lpLBoxInfo,wIndex,LIF_BYPOSITION);
		if (!(lpLBoxInfo->wLBoxStyle & LBS_COMBOLBOX) &&
		     (wIndex != wIndexOld))	
		   LBoxNotify(lpLBoxInfo,LBN_SELCHANGE,TRUE);
	...
	case WM_LBUTTONDBLCLK:
	    ...
	    if (wIndex == (WORD)-1)
		return (LRESULT)LB_ERR;
	    wOldSelection = lpLBoxInfo->wSelection;
	    LBoxChangeSelection(lpLBoxInfo,wIndex,LIF_BYPOSITION);
	    if (wIndex != wOldSelection) {
		if (!(lpLBoxInfo->wLBoxStyle & LBS_COMBOLBOX))
		    LBoxNotify(lpLBoxInfo, LBN_SELCHANGE, TRUE);
	    }
	...
	case LB_GETSELITEMS:
	    ...
	    // pLBoxInfo->wSelection is always initialized -1
	    if (lpLBoxInfo->wSelection == (WORD)-1 && lpLBoxInfo->wSelCount == 1)
		return 0L;
		...
		*lpItems = (int)lpLBoxInfo->wSelection;
	    ...
	    // starting index is 0 so can't put "=" sign
...
LBoxCreate(HWND hWnd, LPCREATESTRUCT lpCrS)
{
    ...
    SetWindowWord(hWnd, LWD_HMENU, (WORD)(UINT)(lpLBInfo->ObjHead.hObj));
...
LBoxCalcFrame(LPLISTBOXINFO lpLBInfo)
{
    ...
    wItemHeight = lpLBInfo->wItemHeight;
	...
	if (!wItemHeight && BOWNERDRAW(lpLBInfo)) {
	    mis.CtlType = ODT_LISTBOX;
	    mis.CtlID = (lpLBInfo->hWnd)?
		GetWindowID(lpLBInfo->hWnd):(UINT)lpLBInfo->hListBox;
	    mis.itemID = (UINT)-1;
            mis.itemData = 0;
	    mis.itemHeight = 0;
...
LBoxItemFromCommand(LPLISTBOXINFO lpLBInfo, LPWORD lpwCommand)
{
	...
	if (lpCurrent->hPopupMenu == (HMENU)(UINT)(*lpwCommand)) {
	    *lpwCommand = wIndex;
	    return lpCurrent;
	}
    }
...
LBoxChangeSelection(LPLISTBOXINFO lpLBInfo, WORD wIndex, WORD wFlags)
{
    ...
    bNewSel = (lpLBInfo->wSelection != wOldSelection);

    /* If not a new sel and not MULTIPLESEL, do nothing */
    if (!bNewSel && BSINGLESEL(lpLBInfo))
	return;

    if (wOldSelection != (WORD)-1) {
	lpOldSelection = LBoxItemFromIndex(lpLBInfo, wOldSelection);

	if (!BSINGLESEL(lpLBInfo) && (wIndex != (WORD)-1)) {
	    if (((lpLBInfo->wLBoxStyle & LBS_EXTENDEDSEL) &&
		GetKeyState(VK_CONTROL))
		|| (lpLBInfo->wLBoxStyle & LBS_MULTIPLESEL)) {
		    bKillSel = FALSE;
		}
	    }
	...
	if (bKillSel) {
	    ...
	    if (bNewSel && LBoxTestState(lpLBInfo,LSF_FOCUS))
	    /* kill focus rect on the old selection */
	    LBoxSendItemDraw(lpLBInfo,(UINT)wOldSelection, ODA_FOCUS,0);
	}
    }
	...
	if ( (lpLBInfo->wLBoxStyle & LBS_EXTENDEDSEL) ) {
		...
		if (wOldSelection == (WORD)-1) 
		    wOldSelection = wIndex;
		for (i = min(wOldSelection,wIndex), lpLBItem = LBoxItemFromIndex(lpLBInfo, (WORD) i );
		     i < min(wOldSelection,wIndex)+abs(wOldSelection-wIndex)+1;
		     lpLBItem = lpLBItem->lpNextItem,i++) {
		    LBoxItemSelection(lpLBInfo, lpLBItem, (WORD)i,
		...
		if (GetKeyState(VK_CONTROL)) {
		    if (!(lpLBItem->wItemFlags & LIF_HILITE)) {/* Normal new sel */
			LBoxItemSelection(lpLBInfo, lpLBItem, 
					  lpLBInfo->wSelection,LIF_BYPOSITION|
					  LIF_HILITE, TRUE);
			lpLBInfo->wSelCount++;
			lpLBItem->wItemFlags |= LIF_HILITE;
		    } else { /* Deselect it */
			LBoxSendItemDraw(lpLBInfo,(UINT)lpLBInfo->wSelection,
					 ODA_FOCUS,0);
			LBoxItemSelection(lpLBInfo, lpLBItem, 
					  lpLBInfo->wSelection,
					  LIF_BYPOSITION, FALSE);
			lpLBInfo->wSelCount--;	   			
			/* Find the last one selected and select it */
			wOldSelection = (WORD)-1;
			for (lpLBItem = lpLBInfo->lpListHead,i = 0;
			     lpLBItem;
			     lpLBItem = lpLBItem->lpNextItem,i++) {
			    if (lpLBItem->wItemFlags & LIF_HILITE) {
			...
			lpLBInfo->wSelection = wOldSelection;
			if (lpLBInfo->wSelection != (WORD)-1) {
			    lpLBItem = LBoxItemFromIndex(lpLBInfo, 
							 lpLBInfo->wSelection);
			    LBoxItemSelection(lpLBInfo, lpLBItem, 
					      lpLBInfo->wSelection, 
					      LIF_BYPOSITION|LIF_HILITE, TRUE);
		...
		} else { /* No shift, no ctrl : normal new sel */
			LBoxItemSelection(lpLBInfo, lpLBItem, 
					  lpLBInfo->wSelection,LIF_BYPOSITION|
					  LIF_HILITE, TRUE);
			lpLBInfo->wSelCount++;
	...
	if (  (lpLBInfo->wLBoxStyle & LBS_MULTIPLESEL) ) {
	    if (!(lpLBItem->wItemFlags & LIF_HILITE) ) { /* Normal new sel */
		LBoxItemSelection(lpLBInfo, lpLBItem, lpLBInfo->wSelection,
				  LIF_BYPOSITION|LIF_HILITE, TRUE);
		lpLBInfo->wSelCount++;
		lpLBItem->wItemFlags |= LIF_HILITE;
	    } else { /* It's a selected, MULTIPLESEL : deselect it */
		LBoxSendItemDraw(lpLBInfo,(UINT)lpLBInfo->wSelection,
				 ODA_FOCUS,0);
		LBoxItemSelection(lpLBInfo, lpLBItem, lpLBInfo->wSelection,
				  LIF_BYPOSITION, FALSE);
		...
		lpLBInfo->wSelection = wOldSelection;
		if (lpLBInfo->wSelection != (WORD)-1) {
		    lpLBItem = LBoxItemFromIndex(lpLBInfo, lpLBInfo->wSelection);
		    LBoxItemSelection(lpLBInfo, lpLBItem, lpLBInfo->wSelection,
				      LIF_BYPOSITION|LIF_HILITE, TRUE);
		}
	    }
	}
	if (BSINGLESEL(lpLBInfo)) { /* Normal one */
	    LBoxItemSelection(lpLBInfo, lpLBItem, lpLBInfo->wSelection,
			      LIF_BYPOSITION|LIF_HILITE, TRUE);
	    lpLBInfo->wSelCount++;
	    lpLBItem->wItemFlags |= LIF_HILITE;
...
LBoxResetContent(LPLISTBOXINFO lpLBInfo, BOOL bDeleteData)
{
	...
	/* Do not forget to release, Owner is not required to do so. */
		...
		WORD wFunc)
{
	...
	cis.itemData2 = (bOwnData)?lpLBItem->dwItemData:
	        (DWORD)lpLBItem->lpItemString;
	if (!(lpLBItem->wItemFlags & LIF_SEPARATOR)) {
...
LBoxModifyItem(LPLISTBOXINFO lpLBInfo, LPMENUITEMSTRUCT lpMis)
{
	...
	if (lpMis->wAction & LCA_SET) {
	    lpLBItem->hPopupMenu = (HMENU)(UINT)(lpMis->wIDNewItem);
	}
	else
	    return MAKELONG((UINT)lpLBItem->hPopupMenu,0);
    }
		...
		LPMENUITEMSTRUCT lpMis)
{
    ...
    lpNewItem->hPopupMenu = (HMENU)(UINT)(lpMis->wIDNewItem);
...
#if 0
	    /* unverified patch as per JOG */
	    /* inserting an item, but should we select it? */
	    lpLBInfo->wSelection++;
#endif
	    wItemIndex = 0;
	    break;
	...
	Figure out what this means
	    ...
	    if (lpLBInfo->wSelection != (WORD)-1 && wItemIndex <= lpLBInfo->wSelection)
			lpLBInfo->wSelection++;
	}
	break;
	default:
...
LBoxAPI(HMENU32 hMenu32, UINT uiAction, LPARAM lParam)
{
	...
	case LBA_CREATE:
	    ...
	    return MAKELONG((UINT)hListBox,0);
	...
	case LBA_DESTROY:
		...
		hListBox = lpLBInfo->ObjHead.hObj;

		// free the object pointer we have
    	        RELEASELBOXINFO(lpLBInfo);

		// let object manager do its thing...
	    	return (LONG)(FREELBOX(hListBox))?1:0;
	    }
