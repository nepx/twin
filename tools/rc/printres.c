...
print_control_data(CONTROL_NODE *node, int id, int count)
{
  ...
  while (count--)
	...
	ptr->x, ptr->y, ptr->cx, ptr->cy));
      COUTPUT(( "0x%08lx, 0x%08lx, 0x%08lx, ",
	ptr->dwID, ptr->lStyle, ptr->dwExtStyle));    
      print_num_or_str(ptr->szClass);
      COUTPUT(( ", "));
      print_num_or_str(ptr->szText);
      COUTPUT(( ", "));
      COUTPUT(( "0x%08lx, ", ptr->dwHelpId));
      COUTPUT(( "NULL ")); /* FIXME:  Write out the bData */
      COUTPUT(( " ),\n"));
      node = node->next;
    } 
  print_last_entry(8);
...
void print_dialog_res(int i, NAMEINFO *info)
{
  ...
  COUTPUT(( " 0x%08lx, 0x%08lx, %d, 0x%08lx, 0x%04x, ",
	dlg_data->lStyle, dlg_data->dwExtStyle,
	dlg_data->dialogEx, dlg_data->dwHelpId,
	dlg_data->bNumberOfItems));
  ...
  COUTPUT((", "));
  COUTPUT(( "0x%04x, ", dlg_data->wWeight));
  COUTPUT(( "0x%04x, ", dlg_data->wItalic));
...
void print_resources(void)
{
  ...
  COUTPUT(("/* TWIN Resource Compiler Version %d.%d.%d */\n",
	RC_VER_MAJOR, RC_VER_MINOR, RC_VER_PATCH));
  COUTPUT(("\n"));
  COUTPUT(("#define NOAPIPROTO\n"));
  COUTPUT(("\n"));
  COUTPUT(("#include \"windows.h\"\n"));
  COUTPUT(("#include \"Resources.h\"\n"));
  COUTPUT(("#include \"twinrt.h\"\n"));
  COUTPUT(("\n"));
