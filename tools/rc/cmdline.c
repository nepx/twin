...
void usage(void)
{
  fprintf(stderr, "Usage: rc [-a align] [-D defineval] [-e [big|little]]\n"
                  "          [-E entry-function-name] [-h] [-I include-path]\n"
                  "          [-i] [-l] [-m] [-n table-name] [-o output-file-name]\n"
                  "          [-l library] [-U undefineval] [-v] [-V] [-?]\n");
  exit(1);
}
...
void open_files(void)
{
  ...
  /* Check the input filename and append .rc if there is no extension. */
  if (!strchr(in_name,'.'))
    strcat(in_name, ".rc");
