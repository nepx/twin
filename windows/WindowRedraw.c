...
DWORD
InternalSetRedraw(HWND hWnd,WPARAM flag)
{
    HWND32 hWnd32;
    DWORD  rv = 0;
    ...
    if(hWnd32) {
	if(flag) {
		/* clear the no redraw bit */
		hWnd32->dwWinFlags &= ~ WFNOREDRAW;
	} else {
		/* set the no redraw bit */
		hWnd32->dwWinFlags |=   WFNOREDRAW;
	}
    }
    return rv;
...
{
    BOOL bFlag;
    APISTR((LF_APICALL,"RedrawWindow(HWND=%x,RECT=%p,HRGN=%x,UINT=%x)\n",
    ...
    bFlag = TWIN_RedrawWindow(hWnd, lprcUpdate, hrgnUpdate, fuRedraw);

    APISTR((LF_APIRET,"RedrawWindow: returns BOOL %d\n",bFlag));
    return bFlag;
}
...
InternalSetPaintFlags(HWND hWnd, UINT uAction)
{
	...
	QueueSetFlags(hWnd32->hTask, QFPAINT);

	if (hWnd32->hWndParent && hWnd32->dwStyle & WS_CHILD) {
...
InternalPaintWindows(HWND hWnd, UINT uAction)
{
    ...
    /* if the NOREDRAW bit is set, ignore the dirty flags... */
    dwDirty = hWnd32->dwWinFlags;
    if (hWnd32->dwWinFlags & WFNOREDRAW)
	dwDirty = 0;

    if (dwDirty & (WFBKERASE|WFDIRTY|WFNCDIRTY)) {
...
PropagateExpose(HWND32 hWndFrame32, int nFunc, LPVOID lpStruct)
{
    ...
    HWND hWndChild;
    LPRECT lprcExpose;
    REGION32 RgnExpose32,RgnClient32,RgnFrame32,RgnTemp32;
	...
	case COMPLEXREGION:
	    ...
	    for (hWndChild = hWndClient32->hWndChild; hWndChild;
			hWndChild = GetWindow(hWndChild,GW_HWNDNEXTSIB)) {
		hWndChildFrame32 = GETHWND32(GetWindowFrame(hWndChild));
		RgnTemp32 = (REGION32)DRVCALL_REGIONS(PRH_CREATEREGION,0,0,0);
			...
			LPSIZE lpOldSize)
{
    ...
    hWnd32Temp = GETHWND32(hWndTopFrame);
    PropagateExpose(hWnd32Temp, COMPLEXREGION,
	(LPVOID)TWIN_InternalGetRegionData(hRgn));
