 ...
 * information.  This file is from wine-981129/windows/DrawFrame.c
 ************************************************************************/
...
BOOL WINAPI DrawEdge( HDC hdc, LPRECT rc, UINT edge, UINT flags )
{
    ...
    BOOL bool;
    APISTR((LF_APICALL,"DrawEdge(HDC=%x,lpRECT=%p(%d,%d,%d,%d),UINT=%x,UINT=%x)\n",
	hdc,rc,
    ...
    if(flags & BF_DIAGONAL)
      bool = UITOOLS95_DrawDiagEdge(hdc, rc, edge, flags);
    else
      bool =  UITOOLS95_DrawRectEdge(hdc, rc, edge, flags);

    APISTR((LF_APIRET,"DrawEdge returns BOOL %d\n",bool));
    return bool;
}
...
static BOOL UITOOLS95_DrawFrameCaption(HDC dc, LPRECT r, UINT uFlags)
{
    ...
    case DFCS_CAPTIONHELP:
        ...
        hf = CreateFont(-SmallDiam, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
        ...
        GetTextExtentPoint(dc, str, 1, &size);

        if(uFlags & DFCS_INACTIVE)
        {
            SetTextColor(dc, GetSysColor(COLOR_BTNHIGHLIGHT));
            TextOut(dc, xc-size.cx/2+1, yc-size.cy/2+1, str, 1);
        }
        SetTextColor(dc, GetSysColor(uFlags & DFCS_INACTIVE ? COLOR_BTNSHADOW : COLOR_BTNTEXT));
        TextOut(dc, xc-size.cx/2, yc-size.cy/2, str, 1);
...
{
    BOOL bool = 0;
    APISTR((LF_APICALL,"DrawFrameControl(HDC=%x,lpRECT=%p(%d,%d,%d,%d),UINT=%x,UINT=%x)\n",
	hdc,rc,
	rc->left, rc->top, rc->right, rc->bottom,
	uType,uState));
    ...
    if(GetMapMode(hdc) != MM_TEXT) {
        APISTR((LF_APIRET,"DrawFrameControl: returns BOOL FALSE\n"));
        return FALSE;
    }
    ...
    {
    case DFC_BUTTON:
      bool = UITOOLS95_DrawFrameButton(hdc, rc, uState);
      break;
    case DFC_CAPTION:
      bool = UITOOLS95_DrawFrameCaption(hdc, rc, uState);
      break;
    case DFC_MENU:
      bool = UITOOLS95_DrawFrameMenu(hdc, rc, uState);
      break;
    case DFC_SCROLL:
      bool = UITOOLS95_DrawFrameScroll(hdc, rc, uState);
      break;
    default:
    ...
    APISTR((LF_APIRET,"DrawFrameControl: returns BOOL %d\n",bool));
    return bool;
}
