...
GetDCEx(HWND hWnd, HRGN hRgnClip, DWORD dwDCXFlags)
{
	...
	if (!(hDC32->lpDrvData = (LPVOID)DRVCALL_DC(PDCH_CREATE,
		hDC32->dwDCXFlags, 0L, (hWndDC)?WIN_GETDRVDATA(hWndDC):0))) {
	    RELEASEDCINFO(hDC32);
	    FREEDC(hDC);
	    if (hWnd32)
		RELEASEWININFO(hWnd32);
    ...
    if (hVisibleRgn)
	rgnVisibleRgnData = TWIN_InternalGetRegionData(hVisibleRgn);
...
ReleaseDC(HWND hWnd, HDC hDC)
{
    ...
    APISTR((LF_APICALL,"ReleaseDC(hWnd=%x,hDC=%x)\n",hWnd,hDC));

    ASSERT_HDC(hDC32,hDC,0);

    if (hWnd  && !(hWnd32 = GETHWND32(hWnd))) {
	RELEASEDCINFO(hDC32);
	APISTR((LF_APIFAIL,"ReleaseDC: returns FALSE\n"));
	return 0;
    }
	...
	RELEASEDCINFO(hDC32);
        ...
        APISTR((LF_APIRET,"ReleaseDC: returns TRUE\n"));
	return 1;
    }
    ...
    rc = DeleteDC(hDC);
    APISTR((LF_APIRET,"ReleaseDC: returns %d\n",rc));
    return rc;
}
...
TWIN_OffsetDCOrigins(HWND32 hWnd32, int oX, int oY)
{
    ...
    for (i = 0; (lpdc = TWIN_GetDCCache(i)); i++) {
	if (lpdc->fBusy &&
	    lpdc->lpDC &&
	    lpdc->lpDC->hWnd &&
	    (lpdc->lpDC->hWnd == hWnd ||
	     IsChild(hWnd,lpdc->lpDC->hWnd)))
	    OffsetDCOrg((HDC)lpdc->lpDC->ObjHead.hObj,oX,oY);
    }
...
TWIN_RecalcVisRgns(HWND32 hWnd32)
{
	...
	if (lpdc->fBusy &&
	    ...
	    (lpdc->lpDC->hWnd == hWnd ||
	     IsChild(hWnd,lpdc->lpDC->hWnd))) {
	    if ((hVisRgn = TWIN_CalcVisRgn(lpdc->lpDC->hWnd,
				lpdc->lpDC->dwDCXFlags))) {
		SelectVisRgn((HDC)lpdc->lpDC->ObjHead.hObj,hVisRgn);
		DeleteObject(hVisRgn);
	    }
	}
    }
