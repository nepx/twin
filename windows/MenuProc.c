...
MenuBarProc(HWND hWnd, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NCCREATE:
		...
		*lptps = *((LPTRACKPOPUPSTRUCT)lpCrS->lpCreateParams);
		...
		lptps->x = lpCrS->x;
		lptps->y = lpCrS->y;
		SetWindowWord(hWnd, LWD_HMENU, (UINT)lptps->hMenu);
	    }
	...
	case WM_LBUTTONDOWN:
		...
		if (nPopupHit == -1) {
		    ...
		    SendMessage(lptps->hWndOwner,WM_MENUSELECT,
				GET_WM_MENUSELECT_MPS((UINT)lptps->hMenu,-1,0));
	...
	case WM_LBUTTONUP:
		...
		nPopupHit = -1;
	    if ((nPopupHit == -1) && PtInRect(&rcRect,pt)) {
		/* rcRect is non-zero implies hMenu32...*/
		wSel = MenuHitTest(hMenu32,&pt);
		mnis.wPosition = (WORD)-1;
		mnis.wAction = LCA_SELECTION;
		wPrevSel = (WORD)LBoxAPI(hMenu32,LBA_GETDATA,(LPARAM)&mnis);
		...
		if ((wSel = (WORD)SendMessage(hPopup,
		    ...
		    hPopupMenu32 = GETMENUINFO(hMenu);
		    mnis.wAction = LCA_GET|LCA_FLAGS;
		    mnis.wItemFlags = MF_BYPOSITION;
		    mnis.wPosition = wSel;
		    lFlags = LBoxAPI(hPopupMenu32,LBA_MODIFYITEM,
	...
	case MM_MENUINIT:
		...
		return 1;
	    if (!(hMenu32 = GETMENUINFO(lptps->hMenu)))
		return 1;
	    wSel = wParam;
	    mnis.wAction = LCA_GET|LCA_FLAGS;
	    mnis.wItemFlags = MF_BYPOSITION;
	...
	case WM_CHAR:
		...
		if ((lFlags & MF_POPUP) &&
		    ...
		    ClientToScreen(hWnd,&pt);
		    SetRect(&rcRect,pt.x,pt.y,0,0);
		    if (CreatePopup(hWnd,lptps,(HMENU)(UINT)wItemID,wSel,
				&rcRect,TRUE))
			SetPopupMenuSel(lptps->hWndOwner,
				lptps->hPopups[0],0,lFlags & MF_SYSMENU);
		}
		...
		if (lFlags & MF_POPUP) {
		    ...
		    if (lptps->uiFlags & TP_MENUBAR)
			ChangeMBSelection(hWnd,GETMENUINFO(lptps->hMenu),
	...
	case WM_KEYDOWN:
		...
		case VK_RIGHT:
			...
			!(lFlags & (MF_GRAYED|MF_DISABLED|MF_SEPARATOR))) {
			    ...
			    ClientToScreen(hPopup,&pt);
			    SetRect(&rcRect,pt.x,pt.y,0,0);
			    if (CreatePopup(hWnd,lptps,(HMENU)(UINT)wItemID,
					wSel,&rcRect,TRUE))
				SetPopupMenuSel(lptps->hWndOwner,
					lptps->hPopups[lptps->nPopups-1],
					0,lFlags & MF_SYSMENU);
		...
		case VK_RETURN:
			...
			if (lFlags & MF_POPUP) {
			    ...
			    ClientToScreen(hWnd,&pt);
			    SetRect(&rcRect,pt.x,pt.y,0,0);
			    if (CreatePopup(hWnd,lptps,(HMENU)(UINT)wItemID,wPrevSel,
					&rcRect,TRUE))
				SetPopupMenuSel(lptps->hWndOwner,
					lptps->hPopups[0],
					0,lFlags & MF_SYSMENU);
...
DrawPopupMenuItem(HMENU32 hMenu32, HWND hWndOwner, LPDRAWITEMSTRUCT lpdis )
{
	...
	if (lFlags & MF_CHECKED) {
	    ...
	    hCheck = (HBITMAP)LBoxAPI(hMenu32,LBA_MODIFYITEM,(LPARAM)&mnis);
	    DrawCheckMark(lpdis,lFlags,LOWORD(dwIndents),hCheck,
				crTextColor,crBkColor);
	}
	else {
	    mnis.wAction = LCA_GET|LCA_UNCHECKBMP;
	    hUncheck = (HBITMAP)LBoxAPI(hMenu32,LBA_MODIFYITEM,(LPARAM)&mnis);
	    if (hUncheck) 
