...
extern BOOL CreateDIBImage(LPTWIN_IMAGEINFO,BITMAPINFOHEADER *,
	RGBQUAD *,LPVOID,UINT);
			...
			LPLSDS_PARAMS lpStruct)
{
    ...
    hpal = hDC32->hPalette;
    lp   = WINMALLOC(sizeof(LOGPALETTE) + sizeof(PALETTEENTRY)*356,0,0);
			...
			LPLSDS_PARAMS lpStruct)
{
    LSDS_PARAMS argptr;
    LSDE_STRETCHDATA *lpsd = &lpStruct->lsde.stretchdata;
    TWIN_IMAGEINFO image,*lpimage = &image;
    BOOL ret;

    memset((LPSTR)lpimage,'\0',sizeof(TWIN_IMAGEINFO));
	...
	LPLSDS_PARAMS lpParam)
{
	...
	if((lpPalette = GETPALETTEINFO(lpDC->hPalette)) == 0) {
		return GDI_ERROR;
	}
...
static int x_scanblt(SCANBLT **scanblt)
{
	...
	if (!*scanblt)
	{
		if (!(*scanblt = (SCANBLT *) WINMALLOC(sizeof(SCANBLT),0,0)))
		{
			return (0);
		}
		LOGSTR((LF_LOG, "x_scanblt: *%p = %p\n", scanblt, *scanblt));
...
static WORLDBLT *alloc_worldblt(int nDstStretchBltMode,
	...
	int nSrcWidth, int nSrcHeight)
{
	WORLDBLT *worldblt = (WORLDBLT *) WINMALLOC(sizeof(WORLDBLT),0,0);
...
static int x_worldblt(WORLDBLT **worldblt, int y)
{
	...
	{
		int y;
		if (!((*worldblt)->y = (SCANBLT **)
		   WINMALLOC((*worldblt)->nSrcHeight * sizeof(SCANBLT *),0,0)))
		{
			return (0);
		}
		(*worldblt)->y -= (*worldblt)->y0;
	...
	LPLSDS_PARAMS lpParams)
{
	 ...
	 * lpParams - (unused)
	 */
	return (DWORD) DRVCALL_COLORS(PLH_GETSYSTEMPALETTE,
		(DWORD) lpDC->lpDrvData,
		(DWORD) dwParam,
		(LPVOID) lpParams);
